// DuNorth Background Script
console.log('[DuNorth] Extension loaded');
...
// Remove notifications to drop permission requirement
function notify(title, message) { console.log(`[DuNorth] ${title}: ${message}`); }
...
async function handleMessage(msg) {
  console.log('[DuNorth] Handle message:', msg);
  if (msg?.type === 'PING') { notify('DuNorth', 'Hello from extension (PING)'); return { ok: true, ext: 'DuNorth', version: '1.0.2' }; }
  if (msg?.type === 'TEST_ECHO') {
    try {
      ...
      notify('DuNorth', 'Echo ok');
      return { ok: true, echo: data };
    } catch (e) {
      notify('DuNorth', 'Echo failed');
  ...
  if (msg?.type === 'SYNC_CANVAS') {
    ...
    if (!token) { notify('DuNorth', 'Auth token failed'); return { ok: false, error: 'Could not obtain auth token' }; }
    try { const result = await handoffCanvasCookieToBackend(apiEndpoint, token, msg.baseUrl); notify('DuNorth', 'Cookie stored successfully'); return { ok: true, ...result }; } catch (e) { notify('DuNorth', 'Cookie handoff failed'); return { ok: false, error: String(e.message || e) }; }
  }
  return { ok: false, error: 'Unknown message' };
}

chrome.runtime.onMessageExternal.addListener((msg, sender, sendResponse) => { console.log('[DuNorth] External message:', msg); handleMessage(msg).then(sendResponse).catch(err => sendResponse({ ok: false, error: String(err.message || err) })); return true; });
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => { console.log('[DuNorth] Internal message:', msg); handleMessage(msg).then(sendResponse).catch(err => sendResponse({ ok: false, error: String(err.message || err) })); return true; });
