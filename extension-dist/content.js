...
async function syncCanvas(userToken, apiEndpoint) {
  console.log('[StudyHackz] 🚀 Starting Canvas sync...');
  ...
  try {
    ...
    console.log(`[StudyHackz] ✅ Authenticated as: ${user.name}`);
    
    const baseUrl = window.location.origin;
    const stats = { courses: 0, assignments: 0, pages: 0, files: 0, announcements: 0 };
    
    // 2. Get courses
    console.log('[StudyHackz] 📚 Fetching courses...');
    ...
    // 3. Get assignments
    console.log('[StudyHackz] 📝 Fetching assignments...');
    let allAssignments = [];
    ...
    // 4. Get pages
    console.log('[StudyHackz] 📄 Fetching pages...');
    let allPages = [];
    for (const course of courses) {
      try {
    ...
    console.log('[StudyHackz] 📁 Fetching files...');
    let allFiles = [];
    for (const course of courses) {
      try {
    ...
    // 6. Get announcements
    console.log('[StudyHackz] 📢 Fetching announcements...');
    ...
    console.log('[StudyHackz] 💾 Sending to backend...');
    const payload = {
      userId: 'from-token',
      baseUrl,
      items: {
    ...
    console.log('[StudyHackz] ✅ Sync complete:', result);
    
    return { success: true, stats, baseUrl, user: user.name };
    
  } catch (error) {
    console.error('[StudyHackz] ❌ Sync failed:', error);
    throw error;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'SYNC_CANVAS') {
    console.log('[StudyHackz] 📨 Received sync command');
    
    syncCanvas(message.userToken, message.apiEndpoint)
      .then(result => {
        console.log('[StudyHackz] ✅ Sending success response:', result);
        sendResponse({ success: true, ...result });
      })
      .catch(error => {
        console.error('[StudyHackz] ❌ Sending error response:', error.message);
...
console.log('[StudyHackz] 🎯 Content script ready for Canvas sync');
