  ...
  try {
    ...
    const emailPromises = validEmails.map(email => {
      const msg = {
        ...
        subject: `${user.name} invited you to join DuNorth - Get $10 off!`,
        html: generateInviteEmailTemplate(user.name, inviteLink),
        text: `${user.name} has invited you to join DuNorth! You'll get $10 off your subscription. Sign up here: ${inviteLink}`
      };
...
function generateInviteEmailTemplate(inviterName: string, inviteLink: string): string {
  return `
    ...
    <head>
      ...
      <title>You're invited to join DuNorth!</title>
    </head>
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="color: #000; font-size: 28px; margin-bottom: 10px;">You're invited to DuNorth!</h1>
        <p style="font-size: 18px; color: #666; margin: 0;">${inviterName} thinks you'd love DuNorth</p>
      </div>
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 12px; margin-bottom: 30px;">
        ...
        <p style="font-size: 16px; text-align: center; margin-bottom: 20px;">
          Get <strong style="color: #000;">$10 off</strong> your first subscription when you join DuNorth through this invitation.
        </p>
        ...
        <div style="text-align: center;">
          <a href="${inviteLink}" 
            ...
            Join DuNorth & Save $10
          </a>
        </div>
      </div>
      
      <div style="margin-bottom: 30px;">
        <h3 style="color: #000; font-size: 20px; margin-bottom: 15px;">What is DuNorth?</h3>
        <p style="font-size: 16px; margin-bottom: 15px;">
          DuNorth is the AI-powered study companion that helps students excel in their courses. With DuNorth, you can:
        </p>
      ...
      <div style="text-align: center; padding: 20px; background: #f0f0f0; border-radius: 8px; margin-bottom: 20px;">
        <p style="font-size: 14px; color: #666; margin: 0;">
          Join 100k+ students who are already using DuNorth to improve their grades!
        </p>
      </div>
      ...
      <div style="text-align: center; font-size: 14px; color: #999;">
        ...
        <p>© 2024 DuNorth. All rights reserved.</p>
      </div>
    </body>
    </html>
  `;
